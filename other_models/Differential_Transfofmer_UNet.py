...
class MultiheadDiffAttn(nn.Module):
    def __init__(
        ...
        self.head_dim = embed_dim // num_heads  // 2 * model_parallel_size
        self.scaling = self.head_dim ** -0.5

        self.q_proj = nn.Linear(embed_dim, embed_dim, bias=True)
        self.k_proj = nn.Linear(embed_dim, embed_dim // self.n_rep, bias=True)
        self.v_proj = nn.Linear(embed_dim, embed_dim // self.n_rep, bias=True)
    ...
    def forward(
        ...
        k = self.k_proj(x)
        v = self.v_proj(x)
        # print(f'q k v size {q.shape} | {k.shape} | {v.shape}')
        # print(self.num_heads, self.head_dim, self.num_kv_heads)
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, dim, heads=8, dim_head=None,
                 ...
                 mhsa=None, prenorm=True, depth = 4, model_parallel_size = 4):
        """
        Args:
        ...
        """
        super().__init__()
        self.mhsa = MultiheadDiffAttn(embed_dim=dim, depth=depth, num_heads=heads,
    ...
    def forward(self, x, mask=None):
        ...
        if self.prenorm:
            # Unpack self.mhsa output if it returns a tuple (e.g., (output, attn_weights))
            mhsa_output = self.mhsa(x)
            # print(f'mhsa {mhsa_output.shape}')
            if isinstance(mhsa_output, tuple):  # Check if the output is a tuple
                mhsa_output = mhsa_output[0]  # Get only the first element if it's a tuple
            y = self.drop(mhsa_output) + x
            out = self.linear(self.norm_2(y)) + y
        else:
            mhsa_output = self.mhsa(x)
            if isinstance(mhsa_output, tuple):  # Check if the output is a tuple
                mhsa_output = mhsa_output[0]  # Get only the first element if it's a tuple
            y = self.norm_1(self.drop(mhsa_output) + x)
            out = self.norm_2(self.linear(y) + y)
        return out


class TransformerEncoder(nn.Module):
    def __init__(self, dim, blocks=6, heads=8, dim_head=None, dim_linear_block=1024, dropout=0, prenorm=True):
        super().__init__()
        self.block_list = [TransformerBlock(dim, heads, dim_head,
                                            dim_linear_block, dropout, prenorm=prenorm, depth=1, model_parallel_size=4) for _ in range(blocks)]
...
class DiTUnet(nn.Module):
    def __init__(self, *, img_dim, in_channels, classes,
                 vit_blocks=8,
                 vit_heads=8,
                 vit_dim_linear_mhsa_block=128,
                 patch_size=3,
                 vit_transformer_dim=128,
        ...
        self.conv1x1 = nn.Conv2d(in_channels=vit_dim, out_channels=classes, kernel_size=1)
    ...
    def forward(self, x, x_data = None):
        ...
        aux_rec = self.conv1x1_rec(y)
        out_cls = out_seg.mean(dim=[2, 3])
        return out_seg, out_cls, aux_rec
