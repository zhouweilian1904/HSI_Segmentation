...
class PScan2DUnified(torch.autograd.Function):
    ...
    def pscan2d(A, X):
        ...
        # Combined Horizontal and Vertical Scan
        for kh in range(num_steps_h):
            for kw in range(num_steps_w):
                T_h = 2 * (Xa.size(2) // 2)
                T_w = 2 * (Xa.size(3) // 2)

                if T_h > 0 and T_w > 0:
                    Aa = Aa[:, :, :T_h, :T_w, :].reshape(B, D, T_h // 2, T_w // 2, 2, 2,
                                                         -1)  # ([100, 64, 2, 2, 2, 2, 32])
                    Xa = Xa[:, :, :T_h, :T_w, :].reshape(B, D, T_h // 2, T_w // 2, 2, 2,
                                                         -1)  # ([100, 64, 2, 2, 2, 2, 32])

                    Xa[:, :, :, :, 1, 1, :].add_(
                        Aa[:, :, :, :, 1, 1, :].mul(Xa[:, :, :, :, 0, 0, :]))  # ([100, 64, 2, 2, 2, 2, 32])
                    Aa[:, :, :, :, 1, 1, :].mul_(Aa[:, :, :, :, 0, 0, :])  # ([100, 64, 2, 2, 2, 2, 32])

                    Aa = Aa[:, :, :, :, 1, 1, :]  # ([100, 64, 2, 2, 32])
                    Xa = Xa[:, :, :, :, 1, 1, :]  # ([100, 64, 2, 2, 32])

        # Combined Down Sweep
        for kh in range(num_steps_h - 1, -1, -1):
            for kw in range(num_steps_w - 1, -1, -1):
                Aa = A[:, :, 2 ** kh - 1: H: 2 ** kh, 2 ** kw - 1: W: 2 ** kw, :]  # ([100, 64, 2, 2, 32])
                Xa = X[:, :, 2 ** kh - 1: H: 2 ** kh, 2 ** kw - 1: W: 2 ** kw, :]  # ([100, 64, 2, 2, 32])
                ...
                if T_h > 0 and T_w > 0:
                    ...
                    Xa[:, :, 1:, 1:, 0, 0, :].add_(
                        Aa[:, :, 1:, 1:, 0, 0, :].mul(Xa[:, :, :-1, :-1, 1, 1, :]))  # ([100, 64, 2, 2, 2, 2, 32])
                    Aa[:, :, 1:, 1:, 0, 0, :].mul_(Aa[:, :, :-1, :-1, 1, 1, :])  # ([100, 64, 2, 2, 2, 2, 32])
    ...
    def backward(ctx, grad_output_in):
        ...
        PScan2DUnified.pscan2d(A, grad_output_b)  # (B, D, H, W, N) * (B, D, H, W, N)
...
class T_Mamaba(nn.Module):
    def __init__(self, channels, image_size, patch_size, dim, depth, emb_dropout
                 , image_original_size, image_token_size):

        super(T_Mamaba, self).__init__()

        self.dim = dim
        # self.pos_embedding = nn.Parameter(torch.randn(1, dim, image_original_size, image_original_size))
        self.pos_embedding = nn.Conv2d(dim, dim, 1)
        ...
        self.vim2d_layers = nn.ModuleList()
        # Append the encoder layers
        for _ in range(depth):
            self.vim2d_layers.append(
                self.vim2d
            )
...
class Mamba2D_block(nn.Module):
    def __init__(self, channels, image_size, patch_size, dim, depth, emb_dropout,
        ...
        self.T_mamba1 = T_Mamaba(channels, image_size, patch_size=patch_size, dim=dim, depth=depth,
                                 emb_dropout=emb_dropout, image_original_size=image_original_size,
...
class Mamba2D_v3(nn.Module):

    @staticmethod
    def weight_init(m):
        if isinstance(m, (nn.Linear, nn.Parameter, nn.BatchNorm1d, Mamba2D_block,
                          Mamba2D_v3, T_Mamaba, VisionEncoderMambaBlock2D)):
            nn.init.kaiming_normal_(m.weight.data)
            nn.init.zeros_(m.bias.data)

    def __init__(self, channels, num_classes, image_size, patch_size=1, dim=16, depth=1, emb_dropout=0.,
                 ):
        super(Mamba2D_v3, self).__init__()
        ...
        self.mamba2d = Mamba2D_block(channels, image_size, patch_size=patch_size, dim=dim, depth=depth,
                                     emb_dropout=emb_dropout, image_original_size=int(image_size // patch_size),
                                     image_token_size=int(image_size // patch_size))

        self.to_patch_embedding = nn.Sequential(
            # nn.Unfold(kernel_size=(patch_size, patch_size), padding=1, stride=1),
        ...
        self.to_patch_embedding_x_data = nn.Sequential(
            # nn.Unfold(kernel_size=(patch_size, patch_size), padding=1, stride=1),
            Rearrange("b c (h p1) (w p2) -> b h w (p1 p2 c)", p1=patch_height, p2=patch_width),
        ...
        self.back = nn.Sequential(
            ...
            nn.ConvTranspose2d(dim, num_classes, kernel_size=3),
        )
        self.back_x_data = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=3),
            nn.ConvTranspose2d(dim, num_classes, kernel_size=3),
        )
        self.rec_x_data = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=3),
            nn.ConvTranspose2d(dim, 1, kernel_size=3),
        )
    ...
    def forward(self, x, x_data=None):
        ...
        x_data = self.to_patch_embedding_x_data(x_data.squeeze(1))

        # Apply the rotations
        part1 = x + x_data
        part2 = torch.rot90(x, 1, [2, 3])  # 90 degrees counterclockwise
        part3 = torch.rot90(x, -1, [2, 3])  # 90 degrees clockwise
        part4 = torch.rot90(x, 2, [2, 3])  # 180 degrees counterclockwise
        # print('part n', part1.shape, part2.shape, part3.shape, part4.shape, )

        part1_x_data = x_data + x
        part2_x_data = torch.rot90(x_data, 1, [2, 3])  # 90 degrees counterclockwise
        part3_x_data = torch.rot90(x_data, -1, [2, 3])  # 90 degrees clockwise
        part4_x_data = torch.rot90(x_data, 2, [2, 3])  # 180 degrees counterclockwise

        tm1_1, tm2_1, tm3_1, tm4_1 = self.mamba2d(part1, part2, part3, part4)
        # print('tm', tm1_1.shape, tm2_1.shape, tm3_1.shape, tm4_1.shape, )

        tm1_1_x_data, tm2_1_x_data, tm3_1_x_data, tm4_1_x_data = self.mamba2d(part1_x_data, part2_x_data, part3_x_data,
                                                                              part4_x_data)

        O_1 = self.WMF(tm1_1, tm2_1, tm3_1, tm4_1)
        O_1_x_data = self.WMF(tm1_1_x_data, tm2_1_x_data, tm3_1_x_data, tm4_1_x_data)
        # print('O', O_1.shape)
        # O_1 = self.tanh(O_1)
        # print('tm', O_1.shape)
        ...
        out_seg = self.back(self.to_latent(O_1))

        out_cls = self.mlp_head(self.to_latent(O_1.mean(dim=2).mean(dim=2)))  # b c w h

        aux_seg = self.back_x_data(self.to_latent(O_1_x_data))

        aux_rec = self.rec_x_data(self.to_latent(O_1_x_data))

        return out_seg, out_cls[:, 1:]
