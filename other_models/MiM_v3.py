import torch
from other_models.multiscan_v3 import snake_flatten, snake_unflatten
...
class PScan(torch.autograd.Function):
    ...
    def pscan(A, X):
        ...
        # print('num steps', num_steps)  # 5
        # num_steps = int(L)

        # up sweep or reduction step
        Aa = A
        Xa = X
        # print('Aa', Aa.shape)  # 100, 64, 49, 32
        # print('Xa', Xa.shape)  # 100, 64, 49, 32

        for k in range(num_steps):  # k=1, 2, 3, 4, 5
            T = 2 * (Xa.size(2) // 2)  # T= 48 24 12 6 2
            # print('T', T)

            Aa = Aa[:, :, :T].view(B, D, T // 2, 2, -1)
            Xa = Xa[:, :, :T].view(B, D, T // 2, 2, -1)
            # print('Aa', Aa.shape)  # ([100, 64, 24, 2, 32])
            # print('Xa', Xa.shape)  # [100, 64, 24, 2, 32]
            ...
            Aa = Aa[:, :, :, 1]
            Xa = Xa[:, :, :, 1]
            # print('Aa', Aa.shape)  # [100, 64, 24, 32]
            # print('Xa', Xa.shape)  # [100, 64, 24, 32]

        # down sweep
        for k in range(num_steps - 1, -1, -1):
            Aa = A[:, :, 2 ** k - 1: L: 2 ** k, :]  # ([100, 64, 49, 32])
            # print('sweep Aa', Aa.shape)
            Xa = X[:, :, 2 ** k - 1: L: 2 ** k, :]  # ([100, 64, 49, 32])
            # print('sweep Xa', Xa.shape)
            ...
            Aa = Aa[:, :, :T, :].view(B, D, T // 2, 2, -1)  # ([100, 64, 24, 2, 32])
            Xa = Xa[:, :, :T, :].view(B, D, T // 2, 2, -1)  # ([100, 64, 24, 2, 32])
            # print('sweep Aa 1', Aa.shape)
            # print('sweep Xa 1', Xa.shape)

            Xa[:, :, 1:, 0, :].add_(Aa[:, :, 1:, 0, :].mul(Xa[:, :, :-1, 1, :]))  # ([100, 64, 12, 2, 32])
            Aa[:, :, 1:, 0, :].mul_(Aa[:, :, :-1, 1, :])  # ([100, 64, 12, 2, 32])
            # print('sweep Aa 2', Aa.shape)
            # print('sweep Xa 2', Xa.shape)
    ...
    def forward(ctx, A_in, X_in):
        ...
        ctx.save_for_backward(A_in, X)  # (B, D, L, N)
    ...
    def backward(ctx, grad_output_in):
        ...
        A_in, X = ctx.saved_tensors  # A_in (B L D N),  X # (B, D, L, N)
        # print('Ain, X', A_in.shape, X.shape)
        # clone tensors
        A = A_in.clone()
        ...
        grad_output_b = grad_output_in.transpose(2, 1)  # (B, D, L, N)

        # reverse parallel scan
        grad_output_b = grad_output_b.flip(2)
        PScan.pscan(A, grad_output_b)  # (B, D, L, N) * (B, D, L, N)

        grad_output_b = grad_output_b.flip(2)
        # print('grad_out_b', grad_output_b.shape) # (B, D, L, N)

        Q = torch.zeros_like(X)  # Q (B, D, L, N)
        # print('Q', Q.shape)
        Q[:, :, 1:, :].add_(X[:, :, :-1, :] * grad_output_b[:, :, 1:, :])

        Q = Q.transpose(2, 1)  # Q (B, L, D, N)
        grad_output_b = grad_output_b.transpose(2, 1)  # (B, L, D, N)
...
class T_Mamaba(nn.Module):
    def __init__(self, channels, image_size, patch_size, dim, depth, emb_dropout
                 , seq_length, num_tokens, clstoken: bool = False):
        ...
        self.vim = VisionEncoderMambaBlock(dim=dim, dt_rank=dim, dim_inner=dim, d_state=int(dim / 2),
...
class MiM_v3(nn.Module):
    ...
    def __init__(self, channels, num_classes, center_pixel: bool, image_size: int, patch_size=1, dim=64, depth=2,
                 emb_dropout=0.,
                 ):
        ...
        #                        emb_dropout=emb_dropout, seq_length=7 ** 2 +1, num_tokens=5 ** 2 +1)
        # self.mim_3 = MiM_block(channels, image_size, patch_size=patch_size, dim=dim, depth=depth,
        #                        emb_dropout=emb_dropout, seq_length=5 ** 2 +1, num_tokens=3 ** 2 +1)
        # self.mim_4 = MiM_block(channels, image_size, patch_size=patch_size, dim=dim, depth=depth,
        #                        emb_dropout=emb_dropout, seq_length=3 ** 2 +1, num_tokens=1 ** 2)
    ...
    def forward(self, x, x_data = None):
        ...
        if self.center_pixel:
            ...
            tm1_1 = snake_unflatten(tm1_1, original_shape=(b, self.dim, w, h))
            tm2_1 = snake_unflatten(tm2_1, original_shape=(b, self.dim, w, h))
            tm3_1 = snake_unflatten(tm3_1, original_shape=(b, self.dim, w, h))
            tm4_1 = snake_unflatten(tm4_1, original_shape=(b, self.dim, w, h))
            O_1 = self.WMF(tm1_1, tm2_1, tm3_1, tm4_1)
        ...
        if self.center_pixel:
            ...
            b, c, w, h = O_1.shape
            out_cls = O_1[:, :, w // 2, h // 2]
            return self.back(self.to_latent(O_1)), self.mlp_head(out_cls)  # b c w h
