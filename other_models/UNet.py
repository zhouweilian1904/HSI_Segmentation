...
class UNet(nn.Module):
    def __init__(self, n_channels, x_n_channels, n_classes, bilinear=True, type = 'seg'):
        ...
        self.inc_x_data = (DoubleConv(x_n_channels, 64))
        ...
        self.nn1 = nn.Linear(64, n_classes, bias=True)
    ...
    def forward(self, x, x_data):
        ...
        x2 = self.down1(x1)
        x_data2 = self.down1(x_data1)

        x3 = self.down2(x2)
        x_data3 = self.down2(x_data2)

        x4 = self.down3(x3)
        x_data4 = self.down3(x_data3)

        x5 = self.down4(x4)
        x_data5 = self.down4(x_data4)

        x = self.up1(x5, x4)
        x_data = self.up1(x_data5, x_data4)

        x = self.up2(x, x3)
        x_data = self.up2(x_data, x_data3)

        x = self.up3(x, x2)
        x_data = self.up3(x_data, x_data2)

        x = self.up4(x, x1)
        x_data = self.up4(x_data, x_data1)
        final = x + x_data
