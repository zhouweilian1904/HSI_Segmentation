...
from my_models.Mamba2D_v1 import Mamba2D_v1
from my_models.Mamba2D_v2 import Mamba2D_v2
from my_models.Mamba2D_v3 import Mamba2D_v3
...
def get_model(name, **kwargs):
    ...
    if name == "nn":
        ...
        center_pixel = True
        lr = kwargs.setdefault('lr', 0.0005)
        model = Mamba3D(n_bands, n_classes, patch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters(), lr=lr)
        criterion = (
            nn.CrossEntropyLoss(weight=kwargs["weights"]),
        ...
        # criterion = (nn.CrossEntropyLoss(weight=kwargs['weights']).to(device, dtype=torch.float32),
        #              lambda rec, data: F.mse_loss(rec, data[:, :, :, patch_size // 2, patch_size // 2].squeeze()))
    elif name == 'Mamba2D-v1':
        kwargs['supervision'] = 'full'
        patch_size = kwargs.setdefault('patch_size', 7)
        center_pixel = True
        lr = kwargs.setdefault('lr', 0.0005)
        model = Mamba2D_v1(n_bands, n_classes, patch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters(), lr=lr)
        criterion = (
            nn.CrossEntropyLoss(weight=kwargs["weights"]),
        ...
        # criterion = (nn.CrossEntropyLoss(weight=kwargs['weights']).to(device, dtype=torch.float32),
        #              lambda rec, data: F.mse_loss(rec, data[:, :, :, patch_size // 2, patch_size // 2].squeeze()))
    elif name == 'Mamba2D-v2':
        kwargs['supervision'] = 'full'
        patch_size = kwargs.setdefault('patch_size', 7)
        center_pixel = True
        lr = kwargs.setdefault('lr', 0.0005)
        model = Mamba2D_v2(n_bands, n_classes, patch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters(), lr=lr)
        criterion = (
            nn.CrossEntropyLoss(weight=kwargs["weights"]),
        ...
        # criterion = (nn.CrossEntropyLoss(weight=kwargs['weights']).to(device, dtype=torch.float32),
        #              lambda rec, data: F.mse_loss(rec, data[:, :, :, patch_size // 2, patch_size // 2].squeeze()))
    elif name == 'Mamba2D-v3':
        kwargs['supervision'] = 'full'
        patch_size = kwargs.setdefault('patch_size', 7)
        center_pixel = False
        lr = kwargs.setdefault('lr', 0.001)
        model = Mamba2D_v3(n_bands, n_classes, patch_size)
        model = model.to(device)
        optimizer = optim.AdamW(model.parameters(), lr=lr)
        criterion = (
            nn.CrossEntropyLoss(weight=kwargs["weights"]),
