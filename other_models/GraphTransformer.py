...
class GraphTransformer(nn.Module):
    def __init__(
        ...
        self.conv = nn.Conv2d(in_channels, dim, kernel_size = 1, bias = False)
        self.adj_emb = nn.Embedding(2, edge_dim) if accept_adjacency_matrix else None
        self.edges = torch.randn(1, image_size**2, image_size**2, edge_dim, device = 'cpu' if not torch.cuda.is_available() else 'cuda')
        self.mask = torch.ones(1, image_size**2).bool()
    ...
    def forward(
        ...
        img = img.squeeze(1)
        img = self.conv(img)
        nodes = img.view(img.shape[0], img.shape[2] * img.shape[3], -1)
        ...
        nodes = nodes.view(batch, -1, seq**0.5, seq**0.5)
        out_seg = self.out_seg(nodes)
        b, c, h, w = out_seg.shape
        out_cls = out_seg[:, 1:, h // 2, w // 2]
