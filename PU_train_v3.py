...
def test(device, net, test_loader):
    """
    Evaluate the model on the test set and extract predictions, features, and labels.
    ...
    Returns:
        ...
        features (np.ndarray): Extracted features for visualization or further analysis.
        labels (np.ndarray): Corresponding ground truth labels for the features.
    """
    net.eval()
    y_pred_test = []
    y_test = []
    ...
    with torch.inference_mode():
        for data, x_data, target_patch, target in tqdm(test_loader, desc='Testing on test samples'):
            # Move data to device
            data = data.to(device)
            ...
            predictions = np.argmax(out_cls.detach().cpu().numpy(), axis=1)
            ground_truth = target.detach().cpu().numpy()

            # Append predictions, ground truth, features, and labels
    ...
    return y_pred_test, y_test, features, labels
...
def AA_andEachClassAccuracy(confusion_matrix):
    """
    Calculate per-class accuracy and average accuracy from a confusion matrix.

    Args:
        confusion_matrix (np.ndarray): Confusion matrix (classes x classes).

    Returns:
        each_acc (np.ndarray): Per-class accuracy.
        average_acc (float): Average accuracy across all classes.
    """
    # Diagonal elements (correct predictions)
    diagonal_elements = np.diag(confusion_matrix)

    # Row sums (total samples per class)
    row_sums = np.sum(confusion_matrix, axis=1)

    # Calculate per-class accuracy
    each_acc = np.divide(
        diagonal_elements, row_sums, out=np.zeros_like(diagonal_elements, dtype=float), where=row_sums != 0
    )

    # Calculate average accuracy (mean of per-class accuracies)
    average_acc = np.mean(each_acc)

    return each_acc, average_acc


def acc_reports(y_test, y_pred_test, run):
    target_names = [
        "1.Asphalt",
        "2.Meadows",
        "3.Gravel",
    ...
    TP = np.diag(confusion)  # True Positives are the diagonal elements
    FP = np.sum(confusion, axis=0) - TP  # False Positives are column sums minus TP
    FN = np.sum(confusion, axis=1) - TP  # False Negatives are row sums minus TP
    TN = np.sum(confusion) - (TP + FP + FN)  # True Negatives are the rest

    # Save confusion matrix as a heatmap image
    plt.figure()
    sns.heatmap(confusion, xticklabels=target_names, yticklabels=target_names, linewidths=0.5, linecolor='white')
    plt.xlabel("Predicted Labels")
    plt.ylabel("True Labels")
    plt.savefig(f'confusion_matrix/CM_{run}.png', dpi=600, bbox_inches="tight")
    plt.close()

    # # Generate and save 2×2 matrices for each class
    for i in tqdm(range(len(target_names))):
        print(f"\nClass: {target_names[i]}")
        print(f"  TP: {TP[i]}, FP: {FP[i]}, TN: {TN[i]}, FN: {FN[i]}")
        print(f"  Precision: {TP[i] / (TP[i] + FP[i] + 1e-6):.4f}")
        print(f"  Recall: {TP[i] / (TP[i] + FN[i] + 1e-6):.4f}")
        print(f"  Specificity: {TN[i] / (TN[i] + FP[i] + 1e-6):.4f}")
        print()
        # Create 2×2 matrix for the class
        matrix = np.array([
            [TP[i], FN[i]],  # Top row: TP, FN
            [FP[i], TN[i]]  # Bottom row: FP, TN
        ])
        ...
        sns.heatmap(matrix, annot=True, fmt="d", cbar=True,
                    ...
                    annot_kws={"size": 20, }, linewidths=0.5, linecolor='white')  # Adjust font size for annotations)
        plt.title(f"Metric for {target_names[i]}")
        plt.savefig(f"TP_FP_TN_FN/{target_names[i]}.png", dpi=600, bbox_inches="tight")
        plt.close()
...
if __name__ == '__main__':
    ...
    for run in range(RUN_TIMES):
        ...
        y_pred_test, y_test, features, classes = test(device, net, test_loader)
        ...
        # Evaluation metrics
        classification, oa, confusion, each_acc, aa, kappa = acc_reports(y_test, y_pred_test, run=run + 1)

        print(f"\nRun {run + 1} Results:")
        print(f"OA: {oa}\nAA: {aa}\nKappa: {kappa}\nEach Class ACC: {each_acc}")
        if len(each_acc) < NUM_CLASSES:
            raise ValueError("Missing some class samples. Training is terminated.")
        ...
        file_name = f"cls_result/classification_report_run_{run + 1}.txt"
        with open(file_name, 'w') as x_file:
            x_file.write(f"Run {run + 1}\n")
            x_file.write(f"Training Time (s): {training_time}\n")
            x_file.write(f"Test Time (s): {test_time}\n\n")
            x_file.write(f"Kappa accuracy (%): {kappa}\n")
            x_file.write(f"Overall accuracy (%): {oa}\n")
            x_file.write(f"Average accuracy (%): {aa}\n\n")
            x_file.write(f"Each accuracy (%): {each_acc}\n\n")
            x_file.write(f"Classification Report:\n{classification}\n\n")
